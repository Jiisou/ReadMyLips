...
class VideoPreprocessor:
    def __init__(self, video_path, output_dir, scene_threshold=30, resize_size=(224, 224), fps=30):
        ...
        self.model = models.detection.ssd300_vgg16(pretrained=True).eval()  # SSD 모델 초기화
    ...
    def detect_human_in_frame(self, frame):
        input_tensor = self.transform(frame).unsqueeze(0)  # 배치 차원을 추가
        with torch.no_grad():
            outputs = self.model(input_tensor)

        # SSD 결과에서 사람(class 1)을 찾기
        for box, label in zip(outputs[0]['boxes'], outputs[0]['labels']):
            if label.item() == 1:  # COCO에서 1번 클래스가 'person'
                return True
        return False
...
if __name__ == "__main__":
    video_preprocessor = VideoPreprocessor(video_path='TalkMedia_talkv_high.mp4.mp4', output_dir='extracted_ssd')
